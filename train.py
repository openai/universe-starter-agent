...
parser.add_argument('-g', '--gpu', type=str, default=None, 
                    help="Specify CUDA_VISIBLE_DEVICES. Set to 'CPU' to use CPU only.")
...
def create_tmux_commands(session, num_workers, env_id, logdir, gpu):
    base_cmd = ['python', 'worker.py', '--log-dir', logdir, '--env-id', env_id, '--num-workers', str(num_workers)]
    if gpu is not None:
        if gpu.lower() == 'cpu':
            gpu = ''
        base_cmd = ['CUDA_VISIBLE_DEVICES='+gpu] + base_cmd
...
def run():
    ...
    cmds = create_tmux_commands("a3c", args.num_workers, args.env_id, args.log_dir, args.gpu)
    print("\n".join(cmds))
    os.system("\n".join(cmds))
